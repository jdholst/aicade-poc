...
If a loop is `waiting_for_manual_qa`, use the top-level `review` and verdict commands first. When several parallel candidates are queued, each approval or non-terminal denial leaves the campaign and loop at `waiting_for_manual_qa` until every queued candidate has an explicit verdict. Only an empty queue returns a continuing campaign to `running`; discovery denial or the third qualifying failure returns it to `waiting_for_fix`. Calling `loop resume` before a verdict fails closed.

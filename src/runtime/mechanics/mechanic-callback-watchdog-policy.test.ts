...
describe("mechanic callback watchdog policy", () => {
  it("keeps the exact active-work limit separate from Worker delivery grace", () => {
    expect(MECHANIC_CALLBACK_WATCHDOG_DELIVERY_GRACE_MILLISECONDS).toBe(16);
    expect(mechanicCallbackWatchdogDelayMilliseconds(8)).toBe(24);
    expect(mechanicCallbackWatchdogDelayMilliseconds(0)).toBe(16);
  });
});

...
- If scheduled callbacks already implement transient travel, expiry, and interaction, keep fixed_step minimal: use a no-op callback or only the smallest accepted frame-specific mutation. Do not repeat spatial queries, object reads, or motion writes every fixed-step tick, and do not duplicate scheduled work in fixed_step.

...
- For object_motion_write, call capabilities.objects.writeMotion on the mechanic-owned handle with a finite position or velocity mutation that is necessary for accepted behavior. Initial velocity supplied only inside capabilities.objects.create does not count as object_motion_write use. When lifecycle evidence requires positive simulatedDistanceTraveledDelta, write an explicit position different from the owned object's creation or current observed position during a reachable callback before observation. Velocity-only writes and same-position rewrites do not increase simulatedDistanceTraveledDelta.
...
- For a recurring schedule state named next_*_time paired with an integer interval config, compute nextBoundary = (Math.floor(lifecycleInput.simulationTimeMilliseconds / interval) + 1) * interval after each scheduled occurrence. Write nextBoundary to state, then schedule the relative delay nextBoundary - lifecycleInput.simulationTimeMilliseconds. Do not pass the absolute boundary as the delay, write the completed boundary, or derive the next boundary from a spawn counter; each choice creates recurrence drift when the host dispatches a due callback after its nominal boundary.
- When recurrence and owned-object expiry share the one scheduled callback, use that callback as one deterministic lifecycle coordinator. Never schedule separate callbacks for recurrence and individual object expiry, because every scheduled invocation enters the same callback and the host may dispatch multiple overdue entries at the same coarse-step endpoint. At install, persist each object's creation time in immutable properties and schedule only the earliest future recurrence or expiry boundary. On every scheduled invocation, capture the current simulation time once, rediscover and read owned objects in one bounded pass, remember the latest valid creation time before cleanup, destroy every expired object, perform at most one overdue recurrence, update accepted count and sequence state, and schedule exactly one next scheduled callback for the earliest future recurrence or expiry boundary. Determine overdue recurrence from an accepted absolute next_* state when present, or otherwise from the latest persisted creation time observed before cleanup. Never infer callback purpose from current-time modulus, equality to a nominal deadline, callback multiplicity, or endpoint order, and never replay multiple missed recurrences inside one coarse step.
...
- When targetInteractionsDelta is zero, the target mutation must make a finite nonzero change to at least one accepted motion component. A zero vector such as { x: 0, y: 0 } is not a target interaction; use a deterministic nonzero literal or add a deterministic nonzero delta to the observed target motion.`
  ...
  const ownedObjectLifecycleProgressRepairRule = requiresOwnedObjectTravel
    ? `- For an owned_object_lifecycle_progress_after_action or owned_object_lifecycle_progress_after_install failure, preserve the created owned object as active through the progress observation. Ensure createdDelta is positive through a reachable capabilities.objects.create call, actorOriginCreationsDelta matches createdDelta when actor origin is required, simulatedDistanceTraveledDelta is positive through finite nonzero owned-object position displacement, and targetInteractionsDelta is positive when required. To establish travel, write a position different from the owned object's creation or current observed position during a reachable callback before the observation. Velocity-only writes and same-position rewrites do not increase simulatedDistanceTraveledDelta. Do not destroy the created owned object before the progress observation. Defer expiry cleanup to the accepted scheduled callback after the accepted lifetime, and preserve bounded travel and target interaction while the object remains active.`
...
- When recurring generation and owned-object expiry both use time.schedule, consolidate recurrence and per-object expiry into one deterministic scheduled-callback pass and leave only one pending "scheduled" entry for the earliest future boundary. Never infer why the callback ran from current-time modulus or equality. Use the accepted absolute next_* state when available, otherwise remember the latest persisted object creation time before destroying expired objects, perform at most one overdue recurrence, then compute one positive relative delay to the next recurrence or surviving-object expiry. Remove per-object expiry schedules and duplicate recurrence schedules rather than trying to distinguish identical callback IDs at the coarse-step endpoint.

...
- Unless the accepted intent declares an explicit positive batch-size configuration, create at most one owned object per autonomous recurrence. With recurrence interval I and owned-object lifetime L, the maximum simultaneously live population is ceil(L / I), further bounded by the accepted active limit and owned-object maximum. Simulate every listed advance_time step in order, clean up expired objects before recurring creation, and make final owned_object_count, active-count state, and creation-sequence state agree with that same simulation. Never assume the active limit remains full when objects expire between recurrences.
...
- When contradiction reports that an autonomous recurring final count exceeds the lifetime-and-interval population bound, lower the owned_object_count at the exact reported path to that bound or less. Then recompute matching active-count state and creation-sequence state by simulating every listed advance_time step in order, cleaning up expired objects before creating at most one replacement per recurrence. Preserve the accepted interval, lifetime, active limit, recurrence, and cleanup behavior.

...
## Post-ceiling provider-free diagnosis and fix

- Prompt-attempt accounting: unchanged at **20 of 20**. No Attempt 21 was submitted, no OpenAI endpoint was called, and the existing failed Chrome result was left open.
- Recovered causal evidence: the persisted GenerationRun records for Attempts 19 and 20 contain a lifecycle `contained_failure` for `mechanic_install` with diagnostic code `execution_deadline_exceeded` and message `SES Worker contained mechanic_install.` The editor issue projection retained only the enclosing `Foundation install did not complete.` message.
- Corrected classification: the repeated failure was not provider-authored and was not merely Worker startup. The SES adapter applied the conformance probe's 50 ms wall-clock execution deadline to an admitted runtime transaction. Foundation install performs six sequential asynchronous capability round-trips, while the controller separately enforces the trusted 8 ms active-callback CPU budget. Under the loaded editor, legitimate host/Worker transport crossed 50 ms and the adapter terminated the install.
- Red test: a focused SES adapter regression delayed a valid runtime result by exactly 51 ms. Before the fix, the adapter emitted a termination request at 50 ms and the test failed with two execution requests instead of one.
- Durable fix: runtime transactions now use a bounded host/Worker round-trip deadline derived from the admitted operation budget: one 50 ms transport slice for each of the 16 admitted operations plus one fixed slice, for 850 ms total under the Phase 9 budget. The controller's 8 ms active generated-callback limit, capability grant, operation limits, termination deadline, containment, and fail-closed validation are unchanged.
- QA-harness correction: the retained-session browser fixture had drifted from its generated project fixture by hard-coding `entity_player`. It now registers the exact entity selected by the generated artifact binding. This allowed the provider-free browser verifier to reach the intended foundation seam again.
- Verification: the red regression is green; SES adapter, Runtime and Contract Foundation Gate, and production browser-foundation unit suites pass (3 files / 41 tests); the standalone real-browser foundation gate passes all boundaries; and ten repeated calls through the exact production `createBrowserRuntimeFoundation()` seam produced zero lifecycle failures (nine complete passes and one separate browser-heartbeat conformance rejection). Repository-wide verification passes with 112 files / 1,157 tests, ESLint, and the production Next.js build.
- Independent follow-up: repeated production-seam stress exposed an existing 50 ms runtime-iframe heartbeat flake in realm conformance. That failure is distinct from the fixed lifecycle deadline: it occurs at `realm_conformance` and can reject a run before lifecycle begins. It remains fail-closed and is not treated as a successful foundation result.
- Outcome boundary: the code path that blocked Attempts 19 and 20 is fixed and verified without another provider call, but the original goal remains unachieved because the authorized 20-prompt ceiling was already exhausted and no newly generated projectile game rendered.
